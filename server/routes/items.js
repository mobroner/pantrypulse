...
const workerHandlers = {
    async getAll(request) {
        try {
            const result = await db.query('SELECT * FROM freezer_items WHERE user_id = $1', [request.user.id]);
    ...
    async add(request) {
        ...
        try {
            const newItem = await db.query(
                'INSERT INTO freezer_items (user_id, item_name, quantity, category, expiry_date, barcode, group_id, date_added, storage_area_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
                [request.user.id, item_name, quantity, category, finalExpiryDate, barcode, group_id, date_added, storage_area_id]
            );
            return new Response(JSON.stringify(newItem.rows[0]), { headers: { 'Content-Type': 'application/json' } });
        } catch (err) {
            console.error(err.message);
            return new Response('Server Error', { status: 500 });
        }
    ...
    async update(request) {
        ...
        try {
            const updatedItem = await db.query(
                'UPDATE freezer_items SET item_name = $1, quantity = $2, category = $3, expiry_date = $4, barcode = $5, group_id = $6, date_added = $7, storage_area_id = $8 WHERE id = $9 AND user_id = $10 RETURNING *',
                [item_name, quantity, category, expiry_date, barcode, group_id, date_added, storage_area_id, id, request.user.id]
            );
            if (updatedItem.rows.length === 0) {
            ...
            return new Response(JSON.stringify(updatedItem.rows[0]), { headers: { 'Content-Type': 'application/json' } });
        } catch (err) {
            console.error(err.message);
            return new Response('Server Error', { status: 500 });
        }
    ...
    async remove(request) {
        const { id } = request.params;
        try {
            const deleteOp = await db.query('DELETE FROM freezer_items WHERE id = $1 AND user_id = $2 RETURNING *', [id, request.user.id]);
            if (deleteOp.rows.length === 0) {
                return new Response(JSON.stringify({ msg: 'Item not found or user not authorized' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
            }
